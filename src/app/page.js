...
<div className="fixed top-4 md:top-8 w-full z-50 flex justify-center px-4">
  <nav className="flex items-center gap-3 md:gap-8 px-4 py-2.5 bg-black/90 backdrop-blur-xl border border-white/10 rounded-full max-w-full">
    ...
    <div className="flex gap-4 md:gap-8 text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-white/50">
      <button onClick={() => scrollTo('freelas')} className="hover:text-blue-500 transition-colors">
        {t[idioma].portfolio}
      </button>
      {/* LINK LABS ADICIONADO AQUI */}
      <button onClick={() => scrollTo('labs')} className="hover:text-blue-500 transition-colors">
        {t[idioma].labs}
      </button>
      <button onClick={() => scrollTo('sobre')} className="hover:text-blue-500 transition-colors whitespace-nowrap">
        {t[idioma].carreira}
      </button>
    </div>
    <div className="flex gap-2 border-l border-white/10 pl-4">
      ...
      {/* FOOTER - EMAIL POR ÚLTIMO COM MÁXIMA RESPONSIVIDADE */}
      <footer className="relative py-24 md:py-32 bg-[#030303] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className="text-blue-600 font-mono text-[10px] font-bold tracking-[0.4em] uppercase mb-8">
            {t[idioma].cta_footer}
          </p>
          
          <div className="relative group inline-block w-full mb-20">
            {/* Tooltip de feedback centralizado */}
            ...
            {/* Email com tipografia que encolhe para não quebrar em celulares de 320px */}
            <button 
              onClick={copiarEmail} 
              className="font-bebas text-[clamp(1.5rem,8.5vw,7.5rem)] text-white hover:text-blue-500 transition-all tracking-tighter leading-none lowercase break-all sm:break-normal"
            >
